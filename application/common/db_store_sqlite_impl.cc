...
base::ListValue* DBStoreSqliteImpl::GetApplicationEvents(
    const std::string& id) {
  DCHECK(db_initialized_);

  base::DictionaryValue* value = NULL;
  base::ListValue* events = NULL;
  if (!db_->GetDictionary(id, &value) ||
      !value->GetList(ApplicationStore::kRegisteredEvents, &events))
    return NULL;
  return events;
}

bool DBStoreSqliteImpl::SetApplicationEvents(
    const std::string& id,
  ...
  if (!events)
    RemoveApplicationEvents(id);

  SetValue(GetRegisteredEventsPath(id), events);
  return true;
}

bool DBStoreSqliteImpl::RemoveApplicationEvents(
    const std::string& id) {
  DCHECK(db_initialized_);
  ...
  if (!value || !value->Remove(ApplicationStore::kRegisteredEvents, NULL)) {
    LOG(ERROR) << "Can't remove the record" << id
               << " from database cache.";
    return false;
  }

  return Commit(id, ApplicationStore::kRegisteredEvents, NULL, ACTION_DELETE);
}
